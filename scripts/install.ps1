...
# Runs in its own scope, so `irm | iex` leaves nothing set in your session.
& {
    ...
    # A running lokey.exe (the clipboard-clearing helper lives 30 s after a get)
    # cannot be overwritten, but it can be renamed out of the way.
    $old = "$exe.old"
