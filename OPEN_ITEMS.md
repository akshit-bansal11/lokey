...
| `SHA256SUMS.txt` is published with CRLF line endings | `release.yml` writes it with PowerShell `Set-Content`, so `sha256sum -c SHA256SUMS.txt` fails in Git Bash, WSL and on Linux: every filename in it carries a trailing carriage return, and the checker reports "No such file or directory". The checksums themselves are correct. Present in every release so far. | Write the file through `[IO.File]::WriteAllText` with newline-joined text so the published file uses LF, and verify by running `sha256sum -c` on the downloaded file unmodified. | 2026-09-20 |
